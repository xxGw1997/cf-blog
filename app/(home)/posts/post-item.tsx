...
const PostItem = ({ post, idx }: { post: PostType; idx: number }) => {
  return (
    <BlurFade key={post.id} delay={idx * 0.1} inView className="mt-1">
      <article className="min-w-[300px] h-full flex-1 rounded-sm transition-shadow duration-500 group px-8 py-5 flex gap-5 items-center">
        <ArrowRight className="text-primary opacity-0 group-hover:opacity-100 group-hover:animate-move" />
        <Link
        ...
        <div
          className="text-[13px] opacity-70 flex align-bottom pl-2"
